...
function init()
	...
	if (isAnotherterm or isTermux) then
		config.MakeCommand("androidSendSelction", androidSendSelection, config.NoComplete)
		config.AddRuntimeFile("androidtool", config.RTHelp, "help/androidtool.md")
		-- config.TryBindKey("F9", "lua:androidtool.androidSendSelction", false)	
	end
end